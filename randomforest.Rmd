...
We have successfuly shrunk our data down to __ # miRNAs. 
...
We will use a 70/30, 80/20, or 75/25 split between training and testing, meaning that x% of the data is used to train the model, and the trained model is tested on the remaining y%.
...
Let's break this down. The first argument for the sample command is the range we want to select our samples from. In our case, that is all integers from 1 to the total number of rows. The second argument is how many values we want to sample. Because num_rows * (train%) isn't an integer, we want to round it to the nearest integer. Finally, we don't want to draw the same sample twice, so we set replact to FALSE.
